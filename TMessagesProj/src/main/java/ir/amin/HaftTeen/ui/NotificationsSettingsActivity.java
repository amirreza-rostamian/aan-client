...
import ir.amin.HaftTeen.ui.Cells.TextColorCell;
    ...
    public View createView(Context context) {
        ...
        listView.setOnItemClickListener((view, position) -> {
            ...
            if (position == messageAlertRow) {
                ...
                editor.commit();
                updateServerNotificationsSettings(false);
            } else if (position == messageSoundRow || position == callsRingtoneRow) {
                try {
                    SharedPreferences preferences = MessagesController.getNotificationsSettings(currentAccount);
...
//                    }));
                ...
                showDialog(AlertsCreator.createColorSelectDialog(getParentActivity(), 0, false, position == messageLedRow, () -> adapter.notifyItemChanged(position)));
            } else if (position == messagePopupNotificationRow) {
                if (getParentActivity() == null) {
                    return;
                }
                showDialog(AlertsCreator.createPopupSelectDialog(getParentActivity(), NotificationsSettingsActivity.this, false, position == messagePopupNotificationRow, () -> adapter.notifyItemChanged(position)));
            } else if (position == messageVibrateRow || position == callsVibrateRow) {
                if (getParentActivity() == null) {
                    return;
                }
                String key = null;
    ...
    private class ListAdapter extends RecyclerListView.SelectionAdapter {
        ...
        public boolean isEnabled(RecyclerView.ViewHolder holder) {
            ...
            return !(position == messageSectionRow || position == inappSectionRow ||
                    position == eventsSectionRow || position == otherSectionRow || position == resetSectionRow ||
                    position == eventsSectionRow2 ||
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            switch (holder.getItemViewType()) {
                ...
                case 5:
                    ...
                    SharedPreferences preferences = MessagesController.getNotificationsSettings(currentAccount);
                    if (position == messageSoundRow || position == callsRingtoneRow) {
                        String value = null;
                        if (position == messageSoundRow) {
        ...
        public int getItemViewType(int position) {
            if (position == messageSectionRow || position == inappSectionRow ||
                    position == eventsSectionRow || position == otherSectionRow || position == resetSectionRow ||
                    position == callsSectionRow) {
                return 0;
            } else if (position == messageAlertRow || position == messagePreviewRow ||
                    position == inappSoundRow || position == inappVibrateRow ||
                    position == contactJoinedRow || position == pinnedMessageRow ||
                    position == notificationsServiceRow || position == badgeNumberRow ||
                    position == inchatSoundRow || position == androidAutoAlertRow || position == notificationsServiceConnectionRow) {
                return 1;
            } else if (position == messageLedRow) {
                return 3;
            } else if (position == eventsSectionRow2 ||
                    position == inappSectionRow2 || position == otherSectionRow2 || position == resetSectionRow2 ||
                    position == callsSectionRow2) {
                return 4;
            } else if (position == resetNotificationsRow || false && exceptionChats != null && !exceptionChats.isEmpty()) { //TODO
                return 2;
            } else {
                return 5;
            }
