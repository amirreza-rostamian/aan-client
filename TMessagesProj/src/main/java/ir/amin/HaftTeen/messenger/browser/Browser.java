...
package ir.amin.HaftTeen.messenger.browser;
...
import ir.amin.HaftTeen.messenger.CustomTabsCopyReceiver;
...
import ir.amin.HaftTeen.messenger.ShareBroadcastReceiver;
import ir.amin.HaftTeen.messenger.SharedConfig;
import ir.amin.HaftTeen.messenger.UserConfig;
import ir.amin.HaftTeen.messenger.support.customtabs.CustomTabsCallback;
import ir.amin.HaftTeen.messenger.support.customtabs.CustomTabsClient;
import ir.amin.HaftTeen.messenger.support.customtabs.CustomTabsIntent;
import ir.amin.HaftTeen.messenger.support.customtabs.CustomTabsServiceConnection;
import ir.amin.HaftTeen.messenger.support.customtabs.CustomTabsSession;
import ir.amin.HaftTeen.messenger.support.customtabsclient.shared.CustomTabsHelper;
import ir.amin.HaftTeen.messenger.support.customtabsclient.shared.ServiceConnection;
import ir.amin.HaftTeen.messenger.support.customtabsclient.shared.ServiceConnectionCallback;
...
public class Browser {
    ...
    public static void openUrl(final Context context, final Uri uri, final boolean allowCustom, boolean tryTelegraph) {
        ...
        if (tryTelegraph) {
            try {
                ...
                if (host.equals("telegra.ph") || uri.toString().toLowerCase().contains("aan.org/faq")) {
    ...
    public static boolean isPassportUrl(String url) {
        try {
            url = url.toLowerCase();
            if (url.startsWith("tg:passport") || url.startsWith("tg://passport") || url.startsWith("tg:secureid") || url.contains("resolve") && url.contains("domain=aanpassport")) {
                return true;
            }
        } catch (Throwable ignore) {
    ...
    public static boolean isInternalUri(Uri uri, boolean forceBrowser[]) {
        ...
        } else if ("aan.me".equals(host) || "t.me".equals(host) || "telesco.pe".equals(host)) {
            String path = uri.getPath();
            if (path != null && path.length() > 1) {
                path = path.substring(1).toLowerCase();
                if (path.equals("iv")) {
        ...
        } else if("stage.wavecom.io".equals(host) && uri.getPathSegments().contains("joinchat")) {
            return false; // fixme in-app browser uncomment
        }
        return false;
    }
}
