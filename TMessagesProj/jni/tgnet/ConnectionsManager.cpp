...
void ConnectionsManager::select() {
    ...
    if (datacenter != nullptr) {
        if (datacenter->hasAuthKey(ConnectionTypeGeneric, 1)) {
            ...
            int delta = abs((int32_t) (now / 1000) - lastDcUpdateTime);
            if (delta >= DC_UPDATE_TIME) {
                DEBUG_E("updateDcSettings(0, false) lastDcUpdateTime (%u) >= DC_UPDATE_TIME: %u >= %u !!!", lastDcUpdateTime, delta, DC_UPDATE_TIME);
...
void ConnectionsManager::processServerResponse(TLObject *message, int64_t messageId, int32_t messageSeqNo, int64_t messageSalt, Connection *connection, int64_t innerMsgId, int64_t containerMessageId) {
    ...
    if (typeInfo == typeid(TL_new_session_created)) {
        ...
        DEBUG_D("DC messageSalt: %ld", messageSalt);
        datacenter->addServerSalt(salt);
        saveConfig();
...
void ConnectionsManager::initDatacenters() {
    ...
    std::string host = "31.184.226.114";
    int32_t port = 9009;
#else
    std::string host = "31.184.226.114";
    int32_t port = 9009;
    ...
    //       datacenter->addAddressAndPort("163.172.21.221", 9009, 0, "");
    //       datacenters[3] = datacenter;
//    }

//    if (datacenters.find(3) == datacenters.end()) {
//        datacenter = new Datacenter(instanceNum, 3);
//        datacenter->addAddressAndPort(host, 9011, 0, "");
//        datacenters[3] = datacenter;
//    }
//
//    if (datacenters.find(4) == datacenters.end()) {
//        datacenter = new Datacenter(instanceNum, 4);
//        datacenter->addAddressAndPort(host, 9012, 0, "");
...
void ConnectionsManager::setUserId(int32_t userId) {
    scheduleTask([&, userId] {
        ...
        if (currentUserId != userId && userId != 0) {
            DEBUG_D("updateDcSettings(0, false) setUserId");
            updateDcSettings(0, false);
        }
...
void ConnectionsManager::registerForInternalPushUpdates() {
    ...
    DEBUG_D("FCM token: %s", request->token.c_str());
...
void ConnectionsManager::processRequestQueue(uint32_t connectionTypes, uint32_t dc) {
    ...
    if (!unknownDatacenterIds.empty()) {
        DEBUG_D("updateDcSettings(0, false) unknownDatacenterIds");
        updateDcSettings(0, false);
    }
...
void ConnectionsManager::updateDcSettings(uint32_t dcNum, bool workaround) {
    DEBUG_D("updateDcSettings(%u, %s) ", dcNum, workaround ? "true" : "false");
        ...
        if (response != nullptr) {
            ...
            struct DatacenterInfo {
                ...
                void addAddressAndPort(TL_dcOption *dcOption) {
                    ...
                    DEBUG_D("addAddressAndPort id=%d, host=%s, flags=%u, media_only=%s", dcOption->id, dcOption->ip_address.c_str(), dcOption->flags, dcOption->media_only ? "true" : "false");
                    if (dcOption->media_only) {
                        if (dcOption->ipv6) {
                            addresses = &addressesIpv6Download;
                        } else {
                            std::string downloadRes = std::accumulate(addressesIpv4Download.begin(), addressesIpv4Download.end(), std::string(),
                                                                      [](std::string s, TcpAddress &addr) {
                                                                          return addr.address + "," + s;
                                                                      });
                            DEBUG_D("addAddressAndPort DatacenterInfo.addresses = &addressesIpv4Download, addressesIpv4Download: %s", downloadRes.c_str());
                        ...
                        if (dcOption->ipv6) {
                            ...
                            std::string downloadRes = std::accumulate(addressesIpv4.begin(), addressesIpv4.end(), std::string(),
                                                                      [](std::string s, TcpAddress &addr) {
                                                                          return addr.address + "," + s;
                                                                      });
                            addresses = &addressesIpv4;
                            DEBUG_D("addAddressAndPort DatacenterInfo.addresses = &addressesIpv4, addressesIpv4: %s", downloadRes.c_str());
                        }
                    }
...
void ConnectionsManager::authorizeOnMovingDatacenter() {
    ...
    if (datacenter == nullptr) {
        DEBUG_D("updateDcSettings(0, false) datacenter == nullptr");
        updateDcSettings(0, false);
        return;
    }
    datacenter->recreateSessions(HandshakeTypeAll);
...
void ConnectionsManager::applyDatacenterAddress(uint32_t datacenterId, std::string ipAddress, uint32_t port) {
    scheduleTask([&, datacenterId, ipAddress, port] {
        ...
        if (datacenter != nullptr) {
            ...
            DEBUG_D("updateDcSettings(0, false) applyDatacenterAddress(%u, %s, %u)", datacenterId, ipAddress.c_str(), port);
            ...
            for (std::vector<std::unique_ptr<TL_accessPointRule>>::iterator iter = config->rules.begin(); iter != config->rules.end(); iter++) {
                ...
                if (datacenter != nullptr) {
                    ...
                    if (!addresses.empty()) {
                        ...
                        DEBUG_D("updateDcSettings(0, false) applyDnsConfig");
