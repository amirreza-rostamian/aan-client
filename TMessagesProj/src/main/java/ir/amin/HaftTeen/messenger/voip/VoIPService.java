...
import ir.amin.HaftTeen.ui.VoIPFeedbackActivity;
...
public class VoIPService extends VoIPBaseService {
	...
	public void hangUp() {
		int result = currentState == STATE_RINGING || (currentState==STATE_WAITING && isOutgoing) ?
				DISCARD_REASON_MISSED : DISCARD_REASON_HANGUP;
		declineIncomingCall(result, null);
	}

	public void hangUp(Runnable onDone) {
		int result = currentState == STATE_RINGING || (currentState==STATE_WAITING && isOutgoing) ?
				DISCARD_REASON_MISSED : DISCARD_REASON_HANGUP;
		declineIncomingCall(result, onDone);
	}
				...
				&& NotificationManagerCompat.from(this).areNotificationsEnabled())
		{
			...
			try {
				PendingIntent.getActivity(
						...
						new Intent(
								VoIPService.this,
								VoIPActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK),
						0).send();
			} catch (Exception x) {
				if (BuildVars.LOGS_ENABLED) {
