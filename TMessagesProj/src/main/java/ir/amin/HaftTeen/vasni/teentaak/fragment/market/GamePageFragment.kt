...
class GamePageFragment : BaseFragment, NotificationCenter.NotificationCenterDelegate, Callback<JsonObject> {

    private var txt: String = ""
    private var pageId: String = ""
    private val adapter = MoreAdapter()
    private var gameList: List<Tag> = ArrayList()
    private lateinit var refresh_game_page: SwipeRefreshLayout
    private lateinit var rc_game_page: RecyclerView
    private lateinit var empty_game_page_data: View
    private lateinit var pv_game_page_loading: View
    ...
    override fun createView(context: Context?): View {
        ...
        refresh_game_page = fragmentView.findViewById(R.id.refresh_game_page)
        rc_game_page = fragmentView.findViewById(R.id.rc_game_page)
        empty_game_page_data = fragmentView.findViewById(R.id.empty_game_page_data)
        pv_game_page_loading = fragmentView.findViewById(R.id.pv_game_page_loading)

        try {
            initView()
            refresh_game_page.setOnRefreshListener {
                adapter.removeAllData()
                VasniSchema.instance.show(true, pv_game_page_loading)
                refresh_game_page.isRefreshing = true
                initView()
            }
    ...
    override fun onResponse(call: Call<JsonObject>, response: Response<JsonObject>) {
        if (response.isSuccessful && response.body() != null) {
            try {
                refresh_game_page.isRefreshing = false
                VasniSchema.instance.show(false, pv_game_page_loading)
                if (getSuccess(response.body()!!).success == VasniSchema.instance.success) {
                    ...
                    rc_game_page.layoutManager = LinearLayoutManager(fragmentView.context!!)
                    adapter.apply {
                        register(RegisterItem(R.layout.row_cat, GameMediaAdapter::class.java))
                        startAnimPosition(1)
                    }
                    adapter.loadData(gameList)
                    adapter.attachTo(rc_game_page)

                    if (gameList.isEmpty()) {
                        VasniSchema.instance.show(true, empty_game_page_data)
                    } else {
                        VasniSchema.instance.show(false, empty_game_page_data)
                    }
