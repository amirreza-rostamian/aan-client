...
public class ContactsController {
    ...
    public String getInviteText(int contacts) {
        String link = inviteLink == null ? "https://aan.social/dl" : inviteLink;
        if (contacts <= 1) {
    ...
    public void checkAppAccount() {
        ...
        try {
            Account[] accounts = am.getAccountsByType("ir.amin.HaftTeen.messenger");
            systemAccount = null;
        ...
        if (UserConfig.getInstance(currentAccount).isClientActivated()) {
            ...
            if (systemAccount == null) {
                try {
                    systemAccount = new Account("" + UserConfig.getInstance(currentAccount).getClientUserId(), "ir.amin.HaftTeen.messenger");
        ...
        Utilities.globalQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                /*if (schedule) {
                    try {
                        AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
                        Account[] accounts = am.getAccountsByType("ir.amin.HaftTeen.account");
                        boolean recreateAccount = false;
                        if (UserConfig.getInstance(currentAccount).isClientActivated()) {
                            if (accounts.length != 1) {
                                FileLog.e("detected account deletion!");
                                currentAccount = new Account(UserConfig.getInstance(currentAccount).getCurrentUser().phone, "ir.amin.HaftTeen.account");
                            ...
                            Utilities.stageQueue.postRunnable(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    AndroidUtilities.runOnUIThread(new Runnable() {
                                        @Override
                                        public void run() {
                                            mergePhonebookAndaanContacts(phoneBookSectionsDictFinal, phoneBookSectionsArrayFinal, phoneBookByShortPhonesFinal);
    ...
    private void mergePhonebookAndaanContacts(final HashMap<String, ArrayList<Object>> phoneBookSectionsDictFinal, final ArrayList<String> phoneBookSectionsArrayFinal, final HashMap<String, Contact> phoneBookByShortPhonesFinal) {
...
//        query.add(builder.build());
        ...
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.ir.amin.HaftTeen.messenger.android.profile");
        builder.withValue(ContactsContract.Data.DATA1, user.id);
        builder.withValue(ContactsContract.Data.DATA2, "aan Profile");
