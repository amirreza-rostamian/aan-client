...
public class LikeView extends View implements View.OnClickListener {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        Typeface FONT_NAME = Typeface.createFromAsset(context.getAssets(), "fonts/sh_normal.ttf");
    ...
    private void prepareSource() {
        if (hasLike) {
            ...
            if (drawableSelect == null) {
                drawableSelect = getResources().getDrawable(R.drawable.ic_like_red);
            }
            mTextPaint.setColor(selectColor);
            mTextPaintAlphaOut.setColor(selectColor);
        } else {
            if (drawableUnSelect == null) {
                drawableUnSelect = getResources().getDrawable(R.drawable.ic_like_gray);
            }
    ...
    private void changeUI() {
        boolean add = !canCancel || !hasLike;//当前是添加一个赞还是减少一个赞
        if (add) {
            likeCount++;
        } else {
            likeCount--;
        }
        hasLike = !canCancel || !hasLike;
        invalidate();
        requestLayout();
    }
    ...
    private void createLayoutWrapper() {
        ...
        floatingTextWrapper = activity.findViewById(R.id.FloatingText_wrapper);
