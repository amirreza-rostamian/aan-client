...
void Handshake::processHandshakeResponse(TLObject *message, int64_t messageId) {
    ...
    if (typeInfo == typeid(TL_resPQ)) {
        ...
        if (authNonce->isEqualTo(result->nonce.get())) {
            ...
            serverPublicKeys.push_back("-----BEGIN RSA PUBLIC KEY-----\n"
                                       "MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEAo3L9tanq7YQoubrzqsMa\n"
                                       "+6GQBUwp21TIj4rtAby/tUCtxhRn+gWFn/Wq809QlrBF04LjV5sY0Kxv050tiRzX\n"
                                       "Mq0ihab9hfiaIXbbydKw2S4RqEMDhbyk/8Serc3g9qAAWma7CL2F4Rf2RTUPnjJ5\n"
                                       "nnjRhgjBhAjTqcaTYCygzgCIK2zUla1rEq8jWdrAk4Egs+i1MMacymODOKagBI1C\n"
                                       "Kk5wZQDGxS8q8IAtmUFTyf92H65K1+lYtz40JISC3XwwbPSYgsLLOIaBc/1Qv805\n"
                                       "EF7eBmmaaguHFXcPqPY5QFaHSy3uXmHlZ+qGnWHFco0WpDRMYp5/0MrW2uISWd7X\n"
                                       "wwIDAQAB\n"
                                       "-----END RSA PUBLIC KEY-----");
            serverPublicKeysFingerprints.push_back(0xf7143062cd0c9489);
...
void Handshake::onHandshakeConnectionClosed() {
    if (handshakeState == 0 || handshakeState == 1) {
        return;
    }
    needResendData = true;
}
