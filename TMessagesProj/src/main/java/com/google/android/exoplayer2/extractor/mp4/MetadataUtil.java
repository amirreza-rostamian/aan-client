...
/* package */ final class MetadataUtil {
    ...
    private static @Nullable
    CommentFrame parseCommentAttribute(int type, ParsableByteArray data) {
        int atomSize = data.readInt();
        int atomType = data.readInt();
        if (atomType == Atom.TYPE_data) {
            data.skipBytes(8); // version (1), flags (3), empty (4)
    ...
    private static @Nullable
    TextInformationFrame parseIndexAndCountAttribute(
    ...
    private static @Nullable
    TextInformationFrame parseStandardGenreAttribute(
            ParsableByteArray data) {
    ...
    private static @Nullable
    Id3Frame parseInternalAttribute(
