...
import ir.amin.HaftTeen.ui.Components.URLSpanBrowser;
import ir.amin.HaftTeen.ui.Components.URLSpanMono;
import ir.amin.HaftTeen.ui.Components.URLSpanNoUnderline;
import ir.amin.HaftTeen.ui.Components.URLSpanNoUnderlineBold;
...
public class MessageObject {
    ...
    public Boolean isCallObject = false;
    ...
    public MessageObject(int accountNum, TLRPC.Message message, AbstractMap<Integer, TLRPC.User> users, AbstractMap<Integer, TLRPC.Chat> chats, SparseArray<TLRPC.User> sUsers, SparseArray<TLRPC.Chat> sChats, boolean generateLayout, long eid) {
        ...
        FileLog.w("MESSAGE ID: " + message.id);
        if (message instanceof TLRPC.TL_messageService) {
            if (message.action != null) {
                if (message.action instanceof TLRPC.TL_messageActionCustomAction) {
                    ...
                    TLRPC.TL_messageService sm = (TLRPC.TL_messageService) message;
                    FileLog.w("!!! TL_messageActionPhoneCall !!!");
                    FileLog.w("currentUserId: " + sm.from_id);
                    FileLog.w("out: " + sm.out);
                    FileLog.w("fromId: " + sm.from_id);
                    FileLog.w(String.format("toId: channel_id = %d, user_id = %d, chat_id = %d",sm.to_id.channel_id, sm.to_id.user_id, sm.to_id.chat_id));
                    FileLog.w("mentioned: " + sm.mentioned);
                    FileLog.w("silent: " + sm.silent);
                    FileLog.w("post: " + sm.post);
                    FileLog.w("id: " + sm.id);
                    FileLog.w("reply_to_msg_id: " + sm.reply_to_msg_id);
    ...
    public void setType() {
        ...
        if (type == 0 && isCallObject) {//TODO for call history customize
            type = 16;
        }
    }
    ...
    public void generateLayout(TLRPC.User fromUser) {
        ...
        if (messageOwner instanceof TLRPC.TL_message_old) {
            Log.e("ower is", "message_old");
        }
        if (messageOwner instanceof TLRPC.TL_message_old2) {
            Log.e("ower is", "message_old2");
        }
        if (messageOwner instanceof TLRPC.TL_message_old3) {
            Log.e("ower is", "message_old3");
        }
        if (messageOwner instanceof TLRPC.TL_message_old4) {
            Log.e("ower is", "message_old4");
        }
        if (messageOwner instanceof TLRPC.TL_messageForwarded_old) {
            Log.e("ower is", "message_old4");
        }
        if (messageOwner instanceof TLRPC.TL_messageForwarded_old2) {
            Log.e("ower is", "message_old4");
        }
        if (messageOwner instanceof TLRPC.TL_message_secret) {
            Log.e("ower is", "message_old4");
        }
        if (messageOwner.media instanceof TLRPC.TL_messageMediaInvoice) {
            Log.e("ower is", "message_old4");
        }
        if (messageOwner.media instanceof TLRPC.TL_messageMediaUnsupported) {
            Log.e("ower is", "message_old4");
        }
        if (isOut() && messageOwner.send_state != MESSAGE_SEND_STATE_SENT) {
            Log.e("ower is", "message_old4");
        }

        useManualParse = true; /* fixme: useManualParse forced to parse all messages */
