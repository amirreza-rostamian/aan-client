...
import ir.amin.HaftTeen.ui.ChatActivity;
import ir.amin.HaftTeen.ui.Components.AlertsCreator;
import ir.amin.HaftTeen.ui.DialogsActivity;
import ir.amin.HaftTeen.ui.ProfileActivity;
...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public boolean isFromBot = false;
            ...
            for (int a = 0; a < providers.length; a++) {
                ...
                if (mapArgs.length > 0) {
                    ...
                    if (typeAndKey.length > 0) {
                        ...
                        } else if ("aan".equals(typeAndKey[0])) {
                            if (a == 0) {
                                mapProvider = 2;
                            }
                        ...
                        mapKey = "AIzaSyCzBCMAXVE4rvs0wYdo8WIaLaNXsg1ifb0";
                    }
                }
            }
    ...
    public boolean processUpdateArray(ArrayList<TLRPC.Update> updates, final ArrayList<TLRPC.User> usersArr, final ArrayList<TLRPC.Chat> chatsArr, boolean fromGetDifference) {
        ...
        for (int c = 0, size3 = updates.size(); c < size3; c++) {
            ...
            if (baseUpdate instanceof TLRPC.TL_updateNewMessage || baseUpdate instanceof TLRPC.TL_updateNewChannelMessage) {
                ...
                updatesOnMainThread.add(baseUpdate);
            } else if (baseUpdate instanceof TLRPC.TL_call_update) {
                if (updatesOnMainThread == null) {
                    updatesOnMainThread = new ArrayList<>();
                }
                updatesOnMainThread.add(baseUpdate);
            ...
            if (updatesOnMainThreadFinal != null) {
                ...
                for (int a = 0, size = updatesOnMainThreadFinal.size(); a < size; a++) {
                    ...
                    if (baseUpdate instanceof TLRPC.TL_updatePrivacy) {
                        ...
                        handleCallUpdate((TLRPC.TL_call_update)baseUpdate);
                    } else if (baseUpdate instanceof TLRPC.TL_updatePhoneCall) {
    ...
    private void handleCallUpdate(TLRPC.TL_call_update update) {
        String callId = update.call_id;
        CallService curCallService = CallService.getSharedInstance();
        if (update.event instanceof TLRPC.TL_call_invitation) {
            if (curCallService != null) {
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.d("Auto-declining call " + " because there's already active one");
                }
                ...
                ConnectionsManager.getInstance(currentAccount).sendRequest(req, (response, error) -> {
                    if (error != null) {
                        if (BuildVars.LOGS_ENABLED) {
                            Log.d("TL_call_disconnect", error.toString());
                        }
                    } else {
                        if (BuildVars.LOGS_ENABLED) {
                            Log.d("TL_call_disconnect ", response.toString());
                        }
                    }
                }, ConnectionsManager.RequestFlagFailOnServerErrors);
                return;
            ...
            TLRPC.TL_call_callDescription desc = ((TLRPC.TL_call_invitation)update.event).call_description;
            CallService.incomingCall = desc;
            Intent intent = new Intent(ApplicationLoader.applicationContext, CallService.class);
            intent.putExtra("is_outgoing", false);
            intent.putExtra("user_id", desc.participants.get(0));
            intent.putExtra("account", currentAccount);
            try {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    ApplicationLoader.applicationContext.startForegroundService(intent);
        ...
        } else if (update.event instanceof TLRPC.TL_call_participantJoined) {
            TLRPC.TL_call_participantJoined part = (TLRPC.TL_call_participantJoined) update.event;
            CallService callService = CallService.getSharedInstance();
            if (callService == null) {
                FileLog.w("There isn't active call, ignoring update TLRPC.TL_call_participantJoined");
            } else {
                callService.participantJoined(part.call_id, part.participant_id, part.flags);
            }
        } else if (update.signalingMessage instanceof TLRPC.TL_call_sdp) {
            TLRPC.TL_call_sdp sdp = (TLRPC.TL_call_sdp) update.signalingMessage;
            CallService callService = CallService.getSharedInstance();
            if (callService == null) {
                FileLog.w("There isn't active call, ignoring update TLRPC.TL_call_participantJoined");
            } else {
                callService.sdpReceived(sdp);
            }
        } else if (update.signalingMessage instanceof TLRPC.TL_call_iceCandidate) {
            TLRPC.TL_call_iceCandidate candidate = (TLRPC.TL_call_iceCandidate) update.signalingMessage;
            CallService callService = CallService.getSharedInstance();
            if (callService == null) {
                FileLog.w("There isn't active call, ignoring update TLRPC.TL_call_participantJoined");
            } else {
                callService.iceCandidateReceived(candidate);
            }
        } else if (update.event instanceof TLRPC.TL_call_participantDisconnected) {
            TLRPC.TL_call_participantDisconnected part = (TLRPC.TL_call_participantDisconnected) update.event;
            CallService callService = CallService.getSharedInstance();
            if (callService == null) {
                FileLog.w("There isn't active call, ignoring update TLRPC.TL_call_participantDisconnected");
            } else {
                callService.participantDisconnected(part.call_id, part.participant_id);
            }
        } else if (update.event instanceof TLRPC.TL_call_discardParticipant) {
            TLRPC.TL_call_discardParticipant event = (TLRPC.TL_call_discardParticipant) update.event;
            CallService callService = CallService.getSharedInstance();
            if (callService == null) {
                FileLog.w("There isn't active call, ignoring update TLRPC.TL_call_participantDisconnected");
            } else {
                callService.discardParticipant(event.call_id);
            }
        }
    }
    ...
    public void sortDialogs(SparseArray<TLRPC.Chat> chatsDict) {
        ...
        for (int a = 0; a < dialogs.size(); a++) {
            ...
            if (lower_id == selfId) {
                addDialogForward(0, d);
                selfAdded = true;
            } else {
                addDialogForward(-1, d);
            }
        ...
        if (!selfAdded) {
            ...
            if (user != null) {
                ...
                addDialogForward(0, dialog);
            }
        }
    }

    private void addDialogForward(int index, TLRPC.TL_dialog dialog){
        if (index < 0) {
            dialogsForward.add(dialog);
        } else {
            dialogsForward.add(index, dialog);
        }
    }
    ...
    public static void openBotWith(TLRPC.User user, TLRPC.Chat chat, BaseFragment fragment, int type, boolean closeLast) {
        if (user == null && chat == null || fragment == null) {
            return;
        }
        String reason = null;
    ...
    public void openByBotName(String botname, final BaseFragment fragment, final int type) {
        if (botname == null || fragment == null) {
            return;
        }
        TLObject object = getUserOrChat(botname);
        TLRPC.User user = null;
        TLRPC.Chat chat = null;
        if (object instanceof TLRPC.User) {
            user = (TLRPC.User) object;
        ...
        if (user != null) {
            openBotWith(user, null, fragment, type, false);
        } else if (chat != null) {
            openBotWith(null, chat, fragment, 1, false);
        } else {
            if (fragment.getParentActivity() == null) {
                return;
            }
            ...
            final int reqId = ConnectionsManager.getInstance(currentAccount).sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() -> {
                ...
                if (error == null) {
                    ...
                    MessagesStorage.getInstance(currentAccount).putUsersAndChats(res.users, res.chats, false, true);
                    if (!res.chats.isEmpty()) {
                        openBotWith(null, res.chats.get(0), fragment, 1, false);
                    } else if (!res.users.isEmpty()) {
                        openBotWith(res.users.get(0), null, fragment, type, false);
                    }
                } else {
                    if (fragment != null && fragment.getParentActivity() != null) {
                        try {
